 ...
 * Desc 使用KMeans算法根据用户的RFM的值计算客户价值模型+实现模型的保存和加载+K值选取
 *
 */
...
root
 ...
 |-- feature: vector (nullable = true)
    ...
    val kAndSSEArr: Array[(Int, Double)] = map.toArray.sortBy(_._1)
    kAndSSEArr.foreach(println)
    //scala画图不好画,后面单独学的推荐系统中会学习python使用matplotlib画图比较方便
    println("K值选取结束")
    /*
K值选取开始
(3,52.990085240011474)
(4,25.71425812680569)
(5,20.72680848016018)
(6,13.793475146826808)
(7,12.376808480160161)
(8,11.415270018621253)
(9,4.046222399572892)
K值选取结束
     */
    //注意:通过上面的代码演示,发现K值选取6/7/8都较为合适
    //那么问题来了,到底选取几呢?
    //得和产品/运营商量,如产品的设计就是要给用户按照不同价值等级分6个群,那么就应该使用6
    //当然如果产品要求的分群数量太离谱,如3,那么需要和产品商量,建议选取6/7/8,最终决定可能需要商量或者领导拍板决定,不要和产品干起来~
    //=======================肘部法则进行K值选取============================
